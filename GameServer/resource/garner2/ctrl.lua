...
function init_entry(map)
    ...
    SetMapEntryTime(map, "2006/10/18/14/0", "0/6/0", "0/0/30", "0/0/45") --设置入口的时间，地图对象，首次开启时间（年/月/日/时/分），以后再次开启的间隔（日/时/分，全０表示只有首次开启），每次开启到入口消失的间隔（日/时/分，全０表示永不消失），每次开启到地图关闭的间隔（日/时/分，全０表示永不关闭）。。
...
--	SystemNotice(role,How_Many_Active)
	...
	--	local message = "Today combat has "..How_Many_Active .. "players, each being awarded "..Money_Garner2 .."'s gold"
	--	SystemNotice(role,How_Many_Active)
		return
	end
	...
	if How_Many_Active <  6 and How_Many_Active > 1 then
		...
		--local message = "You survived today's combat."..How_Many_Active .."players, each being awarded "..Money_Garner2 .."'s Gold and 1 Saint of the Ring Emblem"
		--Notice ( message )
		return
	end
	...
	if How_Many_Active == 1 then
		...
		local message = "Only survivor in today's combat is "..cha_name.."He obtained  ".. Money_Garner2 .."'s gold and 1 King of the Ring Emblem"
	--	SystemNotice(role,How_Many_Active)
